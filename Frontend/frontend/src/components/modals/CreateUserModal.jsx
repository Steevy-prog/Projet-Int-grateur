    ...
    <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-100">
      ...
      <form className="px-6 py-5 space-y-4" onSubmit={handleSubmit}>
        ...
        <div>
          ...
          <input type="text" required placeholder="ex: operator_marie" value={form.username} onChange={set('username')} className={FIELD} />
        </div>

        <div>
          <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Adresse email</label>
          <input type="email" required placeholder="marie@ferme.ci" value={form.email} onChange={set('email')} className={FIELD} />
        </div>

        <div>
          <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Mot de passe</label>
          <input type="password" required placeholder="••••••••" value={form.password} onChange={set('password')} className={FIELD} />
        </div>
